...
const forLoopTwoToThe = (arr) => {
  let superAwesomeArray = [];
  for (let i = 0; i < arr.length; i++) {
    superAwesomeArray.push(Math.pow(2, arr[i]))
  }
  return superAwesomeArray;
};
...
const forEachTwoToThe = (arr) => {
  let evenCoolerArray = [];
  arr.forEach(value => evenCoolerArray.push(Math.pow(2, value)));
  return evenCoolerArray;
};
...
const extractStats = (arr) => arr.map(value => {
