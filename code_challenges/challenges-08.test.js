...
const filterStringsWithVowels = (arr) => arr.filter(value => value.match(/[aeiou]/g))
...
const getStatName = (arr, minBaseStat) => {
  ...
  }).map(statName => statName.stat.name)
};
...
const evenOddNumericValues = (arr) => arr.filter(value => typeof value === 'number').map(num => {
  if (num % 2) return 'odd';
  else return 'even';
})
