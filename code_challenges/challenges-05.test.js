...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(each => {
    let first = each.slice(each.indexOf(' ') + 1);
    let duo = first.slice(first.indexOf(' ') + 1);
    result.push(duo)
  })
  return result;
};
...
const splitFoods = recipe => recipe.ingredients.map(item => item.replace(/\d*\d\s\w+-*\w+\s/, ''));
...
const totalSumCSV = (str) => {
  let total = 0;
  let subject = str.split(',')
  for (let i = 0; i < subject.length; i++) {
    let num = parseFloat(subject[i])
    total += num;
  }
  return total;
};
