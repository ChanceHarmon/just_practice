...
const mapCurrentEvents = () => currentEvents.news.map(result => new Event(result))
...
const countNumberOfElements = (arr) => arr.reduce((acc, cur, idx) => acc = idx + 1);
...
const reversedString = (str) => str.split('')
  .reduce((acc, cur) => {
    acc.unshift(cur);
    return acc;
  }, [])
...
const countNumberOfChildren = (arr) => arr.reduce((acc, cur) => {
...
const calculateAverage = (arr) => arr.reduce((acc, cur) => {
  acc = acc + cur;
  return acc;
}, 0) / arr.length
...
const countPrimeNumbers = (arr) => arr.reduce((acc, cur) => {
  if (isPrime(cur)) acc++;
  return acc;
}, 0)
...
const extractStat = (statName, arr) => arr.reduce((acc, cur) => {
  if (cur.stat.name === statName) acc = cur
  return acc
}, 0)
...
const extractChildren = (arr) => arr.filter(character => character.name.indexOf('a') >= 0).reduce((accumulator, current) => {
  if (current.children) {
