...
const toTitleCase = (arr) => arr.map(word => `${word.charAt(0).toUpperCase()}${word.slice(1)}`)
...
let biggerThanLuke = (arr) => {
  let string = '';
  let lukesMass = parseInt(starWarsData[0].mass)
  for (let i = 1; i < arr.length; i++) {
    if (!string.length && parseInt(arr[i].mass) > lukesMass) {
      console.log(arr[i].name)
      string += `${arr[i].name}`
    } else if (parseInt(arr[i].mass) > lukesMass) {
      string += ` - ${arr[i].name}`
    }
  }
  return string;
