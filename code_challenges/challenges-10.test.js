...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.send('Why helloo there!')
  })

  app.get('/aboutme', (request, response) => {
    response.send('I don\'t kiss and tell...')
  })

  app.get('/favoritefoods', (request, response) => {
    response.send(['whiskey', 'steak', 'potatoes'])
  })
...
const count = (target, input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    for (let j = 0; j < input[i].length; j++) {
      realArray.push(input[i][j])
    }
  }
  return realArray.filter(value => value === target).length;
};
...
const divisibleByFiveTwoToThePower = (input) => input.map(value => value.filter(valFive => valFive % 5 === 0 && typeof valFive === 'number').map(powerValue => Math.pow(2, powerValue)));
  ...
  data.forEach(value => {
    if (value.gender === 'male' || value.gender === 'female') {
      if (arrayToString.length < 1)
        arrayToString.push(value.name)
      else {
        arrayToString.push(` and ${value.name}`)
      }
...
let findShortest = (data) => {
  let characterHeight = Infinity;
  let character;
  data.forEach(value => {
    if (parseInt(value.height) < characterHeight) {
      characterHeight = parseInt(value.height);
