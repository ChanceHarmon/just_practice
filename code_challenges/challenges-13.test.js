...
const standardizePhoneNumbers = (arr) => arr.map(num => num.replace(/[^0-9]+/g, ''));
...
const unenrollBrook = (arr) => arr.map(first => first.filter(final => !final.includes('Brook')))
...
const characterByIndex = (arr) => arr.map((value, idx) => value.charAt(idx))
